...
HAL_StatusTypeDef BME280_Connect(I2C_HandleTypeDef *hi2c_handle) {
	...
	uint8_t datatowrite;
	datatowrite = 0xB6;
	ret = HAL_I2C_Mem_Write(BME280_I2C, BME280_ADDRESS, RESET_REG, 1, &datatowrite, 1, 1000);
	datatowrite = OSRS_1;
	ret = HAL_I2C_Mem_Write(BME280_I2C, BME280_ADDRESS, CTRL_HUM_REG, 1, &datatowrite, 1, 1000);
	datatowrite = (T_SB_0p5 <<5) |(IIR_16 << 2);
	ret = HAL_I2C_Mem_Write(BME280_I2C, BME280_ADDRESS, CONFIG_REG, 1, &datatowrite, 1, 1000);
	datatowrite = (OSRS_2 <<5) |(OSRS_16 << 2) | MODE_NORMAL;
	ret = HAL_I2C_Mem_Write(BME280_I2C, BME280_ADDRESS, CTRL_MEAS_REG, 1, &datatowrite, 1, 1000);
...
void BME280_Measure (void) {
	if (BMEReadRaw() == 0) {
		...
		else Temperature = (BME280_compensate_T_int32 (tRaw))/100.0;  // as per datasheet, the temp is x100
		...
		else Humidity = (bme280_compensate_H_int32 (hRaw))/1024.0;  // as per datasheet, the temp is x1024

	}
	// if the device is detached
	else Temperature = Pressure = Humidity = 0;
}